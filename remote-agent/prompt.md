...
5. For every mutating action whose success is observable, include a bounded `verification` contract. It must contain 1–4 read-only checks using `text_appeared`, `text_disappeared`, `element_appeared`, `element_disappeared`, `url_matches`, `state_changed`, or `count_changed`. Use `done.verify` for terminal completion as well. A failed or uncertain verification is returned as structured feedback; choose a recovery action instead of repeating the same action unchanged.
